...
- [ ] **[00 — preflight-doctor](plans/00-preflight-doctor.md)** — Host preflight (`+LIBKRUN` build flag, `/dev/kvm`, kvm group, subuid/subgid, cgroups v2, network backend, nested-virt warn) + new `dctl doctor` sibling subcommand + new `docs/INSTALL.md`. Companion edits: `docs/QUICKSTART.md` Prerequisites block, `docs/CLAUDE.md` Quick Orientation block, `dctl test` deprecation banner.
- [ ] **[10 — runtime-adapter-and-lifecycle](plans/10-runtime-adapter-and-lifecycle.md)** — `lib/dctl/runtime/{common,krun}.sh` (the only backend) + `lib/dctl/lifecycle.sh` (the self-owned devcontainer.json interpreter) + close the `init.sh:84` merge-logic gap so `runArgs`/`workspaceMount`/`workspaceFolder` are first-class. Smoke verifies `init.krun` kernel separation.
