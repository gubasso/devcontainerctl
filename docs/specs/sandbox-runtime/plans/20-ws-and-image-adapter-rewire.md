...
Replace every `docker` / `devcontainer` shell-out in the `dctl ws` verbs (`commands/ws/*.sh`) and the `dctl image` verbs (`commands/image/*.sh`) with calls into the `rt_*` interface defined in `lib/dctl/runtime/common.sh`. Also rewire `commands/init/generate_cache.sh` to emit the `runtime: krun` overlay in the cached devcontainer.json (`--runtime krun` in `runArgs` + default krun resource annotations) and to use `rt_image_inspect` for the image-existence check.
...
1. **Refactor every container-CLI / devcontainer-CLI call in `commands/ws/*.sh`** to use `rt_*`:
   ...
   - The bulk-rm path (was `ws.sh:266` pre-reorg → likely `commands/ws/down.sh` post-reorg; verify against the round-15b re-anchor) → `rt_rm`.
2. **Emit the `runtime: krun` overlay in `commands/init/generate_cache.sh`** (was `init.sh:155-203 generate_cached_devcontainer`): append `--runtime krun` to `runArgs` and the Tier 0 flags (`--cap-drop=ALL`, `--security-opt=no-new-privileges`) — note: round 40 is what configures these in the **leaf** `devcontainer.json`; this round just ensures the **cache** is wired to honor them. Default krun resource annotations: `--annotation krun.ram_mib=4096 --annotation krun.cpus=2` (matches ai-agents-sandbox `:301-302`). Make overridable via the `runtime.resources` block — but the schema for that block does not land until round 70 (Phase 7), so for this round hardcode the defaults and leave a `TODO(70)` marker.
3. **Update the image-existence check** in `commands/init/generate_cache.sh` (was `init.sh:237` pre-reorg): the container-CLI `image inspect` → `rt_image_inspect`.
4. **Remove the round-00 Phase-2 deprecation banner from `commands/test/run.sh`** — now that `dctl ws` is podman-only, `dctl test` will work as expected once its own shell-outs are rewired. **However**, `dctl test` itself currently uses `devcontainer up`/`exec` at the test level (was `test.sh:230,253,263` pre-reorg). Choose one of:
...
5. **Preserve worktree mount logic.** `_lib/workspace/git_worktree.sh` (the shared `.git` mount; was `ws.sh:84-104`) is runtime-agnostic and travels through `rt_run`'s extra-args. Verify it still works under the new path; no edits expected.
6. **Preserve label-based container identity.** `_lib/workspace/label_filter.sh` (was `common.sh:82-84`) is reused unchanged — Podman honors `--filter label=...` identically. **Verify before assuming**: if Podman's label-filter semantics differ for `--filter label=K=V` vs `--filter label=K=` (key-only) in a way the `dctl` codebase exercises, patch the helper. The smoke from round 10 already verified the common case.
...
1. **Refactor every container-CLI / build-CLI call in `commands/image/*.sh`** to use `rt_*`:
   - `commands/image/build.sh` legacy build-tool preflight (was `image.sh:149,152` pre-reorg) → `podman info` check inside `rt_build`'s preflight (the round-10 `rt_build` already memoizes the doctor preflight; this is just removing the legacy probe).
   - `commands/image/build.sh` legacy build invocation (was `image.sh:255` pre-reorg) → `podman build` via `rt_build`.
   - `commands/image/list.sh` image-list query (was `image.sh:279` pre-reorg) → `podman images` directly (no new `rt_*` function needed — image listing is a simple wrapper).
2. **Podman build secrets:** the build passes `gh_token` as a secret (was `image.sh:200-211` pre-reorg) to avoid npm rate limits during `mise install`. Use `podman build --secret id=gh_token,src=<path>`. Containerfile `RUN --mount=type=secret,id=gh_token ...` lines are OCI-frontend-compatible and work under Podman/Buildah unchanged. No Containerfile edits required.
...
- Tier 0 hygiene flags **in the leaf devcontainer.json** (`--cap-drop=ALL`, `--security-opt=no-new-privileges`, tmpfs) — round 40's job. This round only wires `generate_cache.sh` to honor whatever is in `runArgs`.
...
`--annotation krun.ram_mib=4096 --annotation krun.cpus=2` should be emitted by `rt_run` in `lib/dctl/runtime/krun.sh`, **not** baked into `generate_cache.sh`. Put it in `rt_run` so a future per-manifest override (round 70) only needs to read the manifest's `runtime.resources` block and pass the values in.

### Re-anchor sanity check

Round 15b should have re-anchored every pre-reorg `file:line` reference in this brief. If you encounter a stale anchor (e.g. `ws.sh:58` instead of `commands/ws/up.sh`), re-anchor it in place during this round's edits and leave a one-line note in the commit message — stage-4 review uses that to flag round-15b incompleteness.
...
- **`generate_cache.sh` emits the runtime overlay but does not enforce Tier 0 flags.** Round 40 adds those to the leaf `devcontainer.json`. Until round 40 ships, `rt_run` will pass `runArgs` that lack `--cap-drop=ALL` — that's expected.
