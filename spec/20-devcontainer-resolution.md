...
resolve_devcontainer_config():
    ...
    if projects.yaml has devcontainer-manifest:         regenerate merged config from manifest layers, return its path
...
- regenerates the merged config fresh from the manifest's layers on every
  invocation (never cached) and returns
  `$XDG_RUNTIME_DIR/dctl/devcontainer/<name>/devcontainer.json`
- participates only when present for the canonical project
- requires `jq`; a merge failure (missing manifest/layer files, or `jq` absent)
  is an error naming the offending manifest
...
- registers the manifest name in `~/.config/dctl/projects.yaml`, applying the
  legacy `devcontainer:` → `devcontainer-manifest:` migration automatically
- runs `dctl test` and prints a final summary (project, devcontainer, image
  status, generated config path, registry path, smoke-test result); exits
  non-zero if the smoke test fails
- does not persist a cached merged config; the merge is regenerated fresh on
...
- does not override `.image` in the generated config
...
- registry manifest cannot be merged (missing manifest/layer files, or `jq`
  absent): fail immediately naming the offending manifest
